 ...
 **********************************************************************/
  ...
  ******************************************************************/
    if(isset($node->field_page_accordeons['und'])): ?>
      <div class="onglets">
        <?php foreach($node->field_page_accordeons['und'] as $block_id): ?>
          ...
          <h3 <?php if($block->field_accordeon_ouvert['und'][0]['value']==1){print "class='ouvert'";}else{print "class='ferme'";}; ?>><?php print $block->field_accordeon_titre['und'][0]['safe_value']; ?><span class="plus-moins"></span></h3>
          <div class="onglet" <?php if($block->field_accordeon_ouvert['und'][0]['value']==0){print "style='display:none;'";} ?>>
  ...
  <?php
     ...
     <div class="cont-galerie">
        ...
        <?php if(sizeof($node->field_page_galerie_photos['und'])>1) $rel=" rel='next'"; ?>
          <?php foreach($node->field_page_galerie_photos['und'] as $file): ?>
            ...
            ?>
            <li><a href="<?php print $url; ?>" data-caption="<?php print $file['title']; ?>" class="galerie"<?php print $rel; ?> title="<?php print $file['title']; ?>"><?php print $thumb; ?> </a></li>
          <?php endforeach; ?>
        </ul>
      </div>
    </div>
    ...
    <?php if(isset($node->field_page_telechargement['und']) || isset($node->field_ressources_documentaires['und']) || isset($node->field_page_telechargement_prive['und'])): ?>
      <div id="telechargement">
        ...
        <div class="content-telechargement">
          <ul>
            ...
            if(isset($node->field_page_telechargement['und'])):
	            foreach($node->field_page_telechargement['und'] as $telechargement):
            ...
            if(isset($node->field_page_telechargement_prive['und'])):
	            foreach($node->field_page_telechargement_prive['und'] as $telechargement):
					$extension = pathinfo($telechargement['filename'], PATHINFO_EXTENSION);
    ...
    *****************************************************/
	?>
	<?php if(isset($node->field_page_liens['und'])): ?>
	<div id="telechargement">
		<h2>Liens utiles</h2>
		<div class="content-telechargement">
			<ul>
				<?php foreach($node->field_page_liens['und'] as $lien): ?>
				<li>
					<a target="<?php print $lien['attributes']['target']; ?>" href="<?php print $lien['url']; ?>">
						<span class="titre"><?php print $lien['title']; ?></span>
					</a>
				</li>
				<?php endforeach; ?>
			</ul>
