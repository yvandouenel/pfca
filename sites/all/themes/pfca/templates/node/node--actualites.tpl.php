...
<div id="node-<?php print $node->nid; ?>" class="<?php print $classes; ?> clearfix"<?php print $attributes; ?>>
  ...
  <div class="content clearfix"<?php print $content_attributes; ?>>
	...
	<?php
		...
		$view = views_get_view('actualites'); // Id de la vue
		$view->set_display('liste_actu'); // Id du display
		...
		<?php
		//teste la provenance:
		if(strstr($_SERVER['HTTP_REFERER'], 'actualites')){
		...
		<div class="news-pager">
			<a class="retour-liste" href="<?php print $href; ?>">Retour aux actualités</a>
    ...
    <?php
      ...
      $date = $node->field_actu_date['und'][0]['value'];
      print '<span class="date">'.format_date(strtotime($date),'date_complete').' -</span>';

      // On affiche les thématiques
      
      //$thematique = taxonomy_term_load($node->field_actu_thematique['und'][0]['tid']);
      //print '<span class="thematique">'.$thematique->name.'</span>';
      
      $i_them = 1;
      $separateur = ", ";
      foreach($node->field_actu_thematique['und'] as $them){
      	$mathem = taxonomy_term_load($them[tid]);
      	
      	if ($i_them == count($node->field_actu_thematique['und'])){ // Dernier élément du tableau
		...
		/* Instructions ici */
      	...
      	echo '<span class="thematique">'.$mathem->name.$separateur.'</span>';
      	$i_them++;
      }
    ...
    <?php if(isset($node->field_actu_telechargement['und']) || isset($node->field_ressources_documentaires['und']) || isset($node->field_actu_telechargement_prive['und'])): ?>
      <div id="telechargement">
        ...
        <div class="content-telechargement">
          <ul>
            ...
            if(isset($node->field_actu_telechargement['und'])):
	            foreach($node->field_actu_telechargement['und'] as $telechargement):
            ...
            if(isset($node->field_actu_telechargement_prive['und'])):
	            foreach($node->field_actu_telechargement_prive['und'] as $telechargement):
					$extension = pathinfo($telechargement['filename'], PATHINFO_EXTENSION);
